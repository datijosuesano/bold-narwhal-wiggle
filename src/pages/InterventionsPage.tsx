  ...
  const filteredInterventions = useMemo(() => {
    ...
    return interventions.filter(item => {
      const matchesSearch = 
        ...
        (item.rit_number || "").toLowerCase().includes(lowerCaseSearch) ||
        (item.assets?.name || "").toLowerCase().includes(lowerCaseSearch) ||
        (item.assets?.location || "").toLowerCase().includes(lowerCaseSearch);

      const matchesStatus = selectedStatus === 'all' || item.invoice_status === selectedStatus;
                  ...
                  {filteredInterventions.map((item) => (
                    <tr key={item.id}>
                      <td className="py-3 px-3 border border-gray-300">
                        ...
                        <div className="text-[10px] text-gray-600 mt-1">{format(new Date(item.intervention_date), 'dd/MM/yyyy')}</div>
                      </td>
                      <td className="py-3 px-3 border border-gray-300">
