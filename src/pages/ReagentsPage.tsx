...
const ReagentsPage: React.FC = () => {
  ...
  const getExpiryStatus = (expiryDate: string | null) => {
    ...
    if (daysLeft <= 30) {
      return { label: `Critique : ${daysLeft} j`, class: "bg-red-500 text-white font-black", level: "critical", critical: true };
    }
    if (daysLeft <= 90) {
      ...
      <div className="grid gap-6 grid-cols-1 md:grid-cols-3">
        ...
        <Card className="shadow-sm border-l-4 border-l-red-600 bg-white">
          ...
          <CardContent>
            ...
            <div className="text-[10px] mt-1.5 space-y-0.5 font-bold tracking-tight">
              {stats.totalExpiredOrExpiring === 0 ? (
                <span className="text-emerald-600 block">✅ Aucun produit à risque (&gt; 6 mois)</span>
              ) : (
                <>
                  {stats.expiredCount > 0 && <span className="text-red-600 block">🔴 {stats.expiredCount} lot(s) périmé(s)</span>}
                  {stats.criticalCount > 0 && <span className="text-red-500 block">🚨 {stats.criticalCount} lot(s) à moins d'un mois</span>}
                  {stats.warningCount > 0 && <span className="text-orange-500 block">⚠️ {stats.warningCount} lot(s) entre 1 et 3 mois</span>}
                  {stats.infoCount > 0 && <span className="text-amber-500 block">🕒 {stats.infoCount} lot(s) entre 3 et 6 mois</span>}
                </>
              )}
            </div>
          </CardContent>
