...
const RegisterPage: React.FC = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      // Déconnexion de sécurité préalable
      ...
      if (data.user) {
        ...
        showSuccess("Votre compte a été créé avec succès ! Connectez-vous maintenant.");
        navigate("/login");
      } else {
      ...
      const msg = err.message || "Impossible d'enregistrer le profil technique.";
      setErrorMessage(msg);
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            {errorMessage && (
              <Alert variant="destructive" className="rounded-xl bg-red-50 border-red-200 text-red-800 animate-in fade-in duration-300">
                ...
                <AlertTitle className="font-bold">Erreur de profil</AlertTitle>
