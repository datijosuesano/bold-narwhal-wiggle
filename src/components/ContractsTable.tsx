...
const getStatusBadge = (contract: Contract) => {
  ...
  if (daysLeft < 0 || contract.status === 'Expired') {
    return <Badge variant="destructive" className="rounded-full">Expiré</Badge>;
  }
  if (daysLeft <= 30 || contract.status === 'ExpiringSoon') {
    return <Badge className="bg-amber-500 hover:bg-amber-600 text-white rounded-full">Échéance Proche</Badge>;
  }
